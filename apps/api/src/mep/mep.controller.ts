...
export class MepController {
  ...
  @Roles(Role.MEP_CONSULTANT, Role.HEAD_ENGINEER, Role.PROJECT_MANAGER, Role.ADMIN)
  @Post('submittals')
  createSubmittal(@Body() dto: CreateMaterialSubmittalDto) {
    return this.mepService.createSubmittal(dto);
  }

  @Roles(Role.HEAD_ENGINEER, Role.PROJECT_MANAGER, Role.MEP_CONSULTANT, Role.ADMIN)
  @Get('submittals')
  findAllSubmittals() {
